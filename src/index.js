...
const DEFAULTS = {
  ...
  // 群组行为监控规则：权重 + 阈值
  // 行为类型：question_mark(发?) / spoiler(剧透遮掩) / self_like(给自己点赞)
  behavior_rules: JSON.stringify({
    question_mark: { weight: 1, enabled: true },
    spoiler: { weight: 3, enabled: true },
    self_like: { weight: 2, enabled: true },
    threshold: 5,
...
async function dbMigrate(env) {
  ...
  const queries = [
    ...
    ON user_behaviors(user_id, created_at)
    `,
    `
    CREATE INDEX IF NOT EXISTS idx_user_behaviors_chat_time
    ON user_behaviors(chat_id, created_at)
    `,
    `
    CREATE INDEX IF NOT EXISTS idx_message_senders_chat_msg
    ON message_senders(chat_id, message_id)
    `
  ];
...
/* -------------------------------------------------------------------------- */
/*                    群组行为监控（发?/剧透/自赞 → 权重 → 禁言）                */
/* -------------------------------------------------------------------------- */

// 获取监控群 ID 列表（逗号分隔的环境变量）
function getMonitorGroupIds(env) {
  const raw = env.MONITOR_GROUP_IDS || '';
  return raw
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
...
function isMonitorGroup(chatId, env) {
  const ids = getMonitorGroupIds(env);
  if (!ids.length) return false;
  return ids.includes(String(chatId));
}

// 获取行为监控规则
async function getBehaviorRules(env) {
  ...
  try {
    ...
    return {
      ...
      threshold: rules.threshold || 5,
      score_window_hours:
        rules.score_window_hours || 24
    };
  } catch {
    return {
      question_mark: { weight: 1, enabled: true },
...
async function setBehaviorRules(rules, env) {
  await setConfig(
    'behavior_rules',
...
// 行为检测：纯问号消息（中英文问号）
function detectQuestionMark(message) {
...
// 行为检测：剧透遮掩文字（Telegram spoiler 实体）
function detectSpoiler(message) {
  const entities =
    message.entities ||
    message.caption_entities ||
    [];
...
// 记录消息发送者映射（用于后续自赞判断）
async function dbRecordMessageSender(
  env,
  chatId,
  messageId,
  userId
  ...
  try {
    await env.TG_BOT_DB.prepare(
      `INSERT OR REPLACE INTO message_senders
...
// 查询消息发送者
async function dbGetMessageSender(
  env,
  chatId,
  messageId
) {
  try {
    const row = await env.TG_BOT_DB.prepare(
      'SELECT user_id FROM message_senders WHERE chat_id = ? AND message_id = ?'
    )
      .bind(
        String(chatId),
        String(messageId)
  ...
  try {
    await env.TG_BOT_DB.prepare(
      `INSERT INTO user_behaviors
       (user_id, chat_id, behavior_type, message_id, weight, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    )
      .bind(
        String(userId),
...
// 计算用户在时间窗口内的累计权重
async function getUserBehaviorScore(
  env,
  userId,
  windowHours
) {
  ...
  try {
    const row = await env.TG_BOT_DB.prepare(
      'SELECT COALESCE(SUM(weight), 0) as total FROM user_behaviors WHERE user_id = ? AND created_at >= ?'
    )
      .bind(String(userId), cutoff)
      .first();
    return row?.total || 0;
  } catch {
    return 0;
...
// 获取用户行为明细（管理命令用）
async function getUserBehaviorDetails(
  env,
  userId,
  limit = 20
) {
  try {
    const { results } =
      await env.TG_BOT_DB.prepare(
        'SELECT * FROM user_behaviors WHERE user_id = ? ORDER BY created_at DESC LIMIT ?'
      )
...
// 处理监控群消息：检测行为并累计权重
async function handleMonitorGroupMessage(
  message,
  env
) {
  ...
  // 已拉黑用户跳过
  if (
    await dbIsBlacklisted(env, userId)
  )
    return;

  // 记录消息发送者映射（用于自赞判断）
  await dbRecordMessageSender(
    env,
    chatId,
    messageId,
    userId
  ...
  const rules = await getBehaviorRules(env);
  let behaviorDetected = false;

  // 检测：发"?"
  if (
    rules.question_mark.enabled &&
    detectQuestionMark(message)
  ) {
  ...
  // 检测：剧透遮掩
  if (
    rules.spoiler.enabled &&
    detectSpoiler(message)
  ) {
    await recordBehavior(
      env,
      userId,
...
// 处理 message_reaction 事件：检测自赞
async function handleMonitorReaction(
  reaction,
  env
) {
  ...
  // 仅处理监控群
  if (!isMonitorGroup(chatId, env)) return;
  ...
  // 已拉黑用户跳过
  if (
    await dbIsBlacklisted(env, reactorId)
  )
    return;

  // new_reaction 必须非空（有新增 reaction）
  const newReaction =
    reaction.new_reaction || [];
  if (!newReaction.length) return;

  // 查询消息发送者，判断是否自赞
  const senderId =
    await dbGetMessageSender(
      env,
      chatId,
      messageId
    );
  ...
  if (!senderId) return;
  if (senderId !== reactorId) return;

  // 确认是自赞
  const rules = await getBehaviorRules(env);

  if (!rules.self_like.enabled) return;
...
// 检查权重是否达到封禁阈值，达到则禁言+黑名单
async function checkBehaviorThreshold(
  env,
  userId,
  chatId,
  rules
  ...
  const score =
    await getUserBehaviorScore(
      env,
      userId,
      rules.score_window_hours
    );

  if (score < rules.threshold) return;
  ...
  try {
    await dbBlacklistAdd(
      env,
      userId,
      `群行为监控触发（累计权重 ${score}/${rules.threshold}）`,
      'behavior_monitor'
    );
  } catch (e) {
    console.error(
  ...
  try {
    await telegramApi(
      ...
      'sendMessage',
      {
        ...
        text:
          `🚨 <b>群行为监控自动封禁</b>\n\n` +
          `👤 用户：<a href="tg://user?id=${userId}">${escapeHtml(userId)}</a>\n` +
          `📊 累计权重：<code>${score}</code> / 阈值 <code>${rules.threshold}</code>\n` +
          `💬 监控群：<code>${escapeHtml(chatId)}</code>\n\n` +
          `行为明细：/admin behavior status ${userId}`,
        parse_mode: 'HTML'
      }
    );
  } catch (e) {
...
// 清理过期的消息发送者映射（保留 24 小时）
async function cleanupMessageSenders(env) {
  const now = Math.floor(
    Date.now() / 1000
  );
  ...
  try {
    await env.TG_BOT_DB.prepare(
      'DELETE FROM message_senders WHERE created_at < ?'
    )
      .bind(cutoff)
      .run();
  } catch (e) {
...
async function handlePrivateMessage(
  ...
  if (
    ...
    // /admin behavior ... — 群行为监控管理
    if (sub === 'behavior') {
      const action = (parts[2] || '').toLowerCase();

      // /admin behavior rules — 查看当前规则
      if (action === 'rules' || !action) {
        const rules = await getBehaviorRules(env);
        const monitorIds = getMonitorGroupIds(env);
        const typeLabel = (t) =>
          t === 'question_mark' ? '发问号(?)' :
          t === 'spoiler' ? '剧透遮掩' :
          t === 'self_like' ? '自赞' : t;
        ...
        let text =
          '📊 <b>群行为监控规则</b>\n\n' +
          `监控群：<code>${escapeHtml(monitorIds.join(', ') || '未设置')}</code>\n` +
          `封禁阈值：<code>${rules.threshold}</code>\n` +
          `统计窗口：<code>${rules.score_window_hours} 小时</code>\n\n` +
          '<b>行为权重：</b>\n';

        for (const t of ['question_mark', 'spoiler', 'self_like']) {
          const r = rules[t];
          text +=
            `• ${typeLabel(t)}：权重 <code>${r.weight}</code> ` +
            `${r.enabled ? '✅' : '❌(已关闭)'}\n`;
        }

        text +=
          '\n<b>可用命令：</b>\n' +
          '<code>/admin behavior status [用户ID]</code> 查看用户行为\n' +
          '<code>/admin behavior threshold &lt;数值&gt;</code> 设置阈值\n' +
          '<code>/admin behavior toggle &lt;类型&gt;</code> 开关行为检测\n' +
          '<code>/admin behavior weight &lt;类型&gt; &lt;数值&gt;</code> 设置权重\n' +
          '<code>/admin behavior reset &lt;用户ID&gt;</code> 清除用户记录\n' +
          '类型：question_mark / spoiler / self_like';
      ...
      if (action === 'status') {
        ...
        if (!targetId || !/^\d+$/.test(targetId)) {
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: '⚠️ 用法：/admin behavior status <用户ID>'
            }
          );
          return;
        }
        ...
        const rules = await getBehaviorRules(env);
        const score = await getUserBehaviorScore(
          env,
          targetId,
          rules.score_window_hours
        );
        const details = await getUserBehaviorDetails(
          env,
          targetId,
          15
        );
        ...
        let text =
          ...
          `📊 累计权重：<code>${score}</code> / 阈值 <code>${rules.threshold}</code>` +
          `${score >= rules.threshold ? ' 🚨已超阈值' : ''}\n` +
          `⏱ 统计窗口：${rules.score_window_hours} 小时\n\n`;
      ...
      if (action === 'threshold') {
        ...
        if (
          ...
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: '⚠️ 用法：/admin behavior threshold <正整数>\n例如：/admin behavior threshold 5'
            }
          );
          return;
        }
      ...
      if (action === 'toggle') {
        ...
        if (!valid.includes(type)) {
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: '⚠️ 用法：/admin behavior toggle <类型>\n' +
                '类型：question_mark / spoiler / self_like'
            }
          );
          return;
        ...
        const rules = await getBehaviorRules(env);
        rules[type].enabled = !rules[type].enabled;
        await setConfig(
          'behavior_rules',
          JSON.stringify(rules),
          env
        ...
        await telegramApi(
          ...
          {
            chat_id: chatId,
            text: `✅ ${type} 检测已${rules[type].enabled ? '开启' : '关闭'}`,
            parse_mode: 'HTML'
          }
        );
        return;
      ...
      if (action === 'weight') {
        ...
        if (
          ...
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: '⚠️ 用法：/admin behavior weight <类型> <非负整数>\n' +
                '例如：/admin behavior weight spoiler 3\n' +
                '类型：question_mark / spoiler / self_like'
            }
          );
          return;
      ...
      if (action === 'reset') {
        ...
        if (!targetId || !/^\d+$/.test(targetId)) {
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: '⚠️ 用法：/admin behavior reset <用户ID>'
            }
          );
          return;
        }
        ...
        try {
          ...
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: `✅ 已清除用户 <code>${escapeHtml(targetId)}</code> 的行为记录`,
              parse_mode: 'HTML'
            }
          );
        } catch (e) {
          await telegramApi(
            ...
            {
              chat_id: chatId,
              text: `❌ 清除失败：${escapeHtml(e?.message || '')}`
            }
          );
        }
        return;
...
async function handleUpdate(update, env) {
  ...
  if (update.message) {
    if (
      ...
      // 监控群消息：检测行为（发?/剧透）并累计权重
      await handleMonitorGroupMessage(
        update.message,
        env
      );
    }
  ...
  // message_reaction 事件：检测自赞行为
  if (update.message_reaction) {
    await handleMonitorReaction(
